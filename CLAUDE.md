...
- **Prod đang LIVE (v1.2.0)**: Vercel (`leave-management-system-nine-phi.vercel.app`, rewrite `/api`) → Railway (`...-production-222f.up.railway.app`, App Sleeping BẬT, root dir `backend`) → Neon Singapore (connection DIRECT, không `-pooler`). Gotchas: (1) **`APP_CORS_ALLOWED_ORIGINS` bắt buộc dù dùng rewrites** — browser luôn gửi `Origin` với POST, thiếu là Spring trả 403 trong khi curl (không Origin) vẫn 200; (2) đừng đặt uptime ping khi App Sleeping bật; (3) Hikari prod max-lifetime 4.5 phút — đừng thêm keepalive (giữ Neon thức, tốn compute); (4) sinh BCrypt trên DB bằng pgcrypto `crypt(pw, gen_salt('bf',12))` khi không pull được image htpasswd; (5) seed prod chỉ MỘT lần bằng cách chạy container profile dev trỏ Neon — DB có user rồi thì seeder tự skip. Secrets nằm trong `.env.neon` (gitignored) + Railway Variables.
