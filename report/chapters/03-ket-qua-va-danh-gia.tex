...
\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{images/dashboard-hr.png}
\caption{Dashboard tổng quan cho HR/Admin}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{images/my-requests.png}
\caption{Màn hình theo dõi đơn nghỉ của người dùng}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{images/approvals.png}
\caption{Màn hình xử lý đơn cần duyệt}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.88\textwidth]{images/notification-bell.png}
\caption{Dropdown thông báo trong ứng dụng}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{images/calendar.png}
\caption{Lịch nghỉ phép có thông tin phòng ban}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{images/reports.png}
\caption{Màn hình báo cáo với bộ lọc và preview}
\end{figure}
...
Frontend unit test & Utility, schema form, component và trạng thái giao diện quan trọng; lần kiểm tra gần nhất ghi nhận 36 test. & Đạt \\
...
\subsection{Đối chiếu yêu cầu, thiết kế và kết quả}
Bảng đối chiếu dưới đây liên kết các nhóm yêu cầu chính với phần thiết kế, hiện thực và bằng chứng kiểm tra. Cách trình bày này giúp chứng minh rằng báo cáo không chỉ mô tả chức năng rời rạc, mà có liên hệ giữa yêu cầu ban đầu và sản phẩm đã xây dựng.

\begingroup
\small
\begin{longtable}{p{0.18\textwidth}p{0.22\textwidth}p{0.30\textwidth}p{0.16\textwidth}}
\toprule
\textbf{Yêu cầu} & \textbf{Thiết kế} & \textbf{Hiện thực} & \textbf{Bằng chứng} \\
\midrule
\endfirsthead
\toprule
\textbf{Yêu cầu} & \textbf{Thiết kế} & \textbf{Hiện thực} & \textbf{Bằng chứng} \\
\midrule
\endhead
Xác thực và phân quyền & JWT, RBAC, kiểm tra phạm vi dữ liệu & Auth API, protected routes, service-level permission checks & Backend test, FE login flow \\
Quản lý dữ liệu nền & Entity người dùng, phòng ban, loại nghỉ, ngày lễ, quỹ phép & Admin/HR console và CRUD API & Backend test, UI quản trị \\
Quản lý đơn nghỉ & State machine, sequence nộp đơn, rule tính ngày & Luồng tạo, sửa, hủy, duyệt và từ chối đơn & Service/API test, smoke test \\
Lịch nghỉ phép & Calendar scope theo vai trò và phòng ban & Calendar API trả department info, FE filter phòng ban/nhân viên & UI calendar, FE build \\
Dashboard và báo cáo & Dashboard theo vai trò, report theo bộ lọc & Dashboard summary, report preview, CSV export & UI screenshots, FE tests \\
Thông báo & Notification gắn vòng đời đơn nghỉ & Notification API, bell dropdown, read/unread filter & UI screenshot, backend test \\
Attachment local-only & Metadata DB, file Docker volume, permission theo đơn & Upload, tải xuống, xóa file và danh sách attachment & Local demo, API test \\
\bottomrule
\end{longtable}
\endgroup
