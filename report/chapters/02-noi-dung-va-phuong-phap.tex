...
Tệp đính kèm local-only & Hệ thống hỗ trợ đính kèm giấy tờ cho đơn nghỉ trong môi trường demo/test local. File chỉ được upload/xóa khi đơn còn ở trạng thái cho phép và không được xem là thành phần triển khai production. \\
...
Các nhóm chức năng trên tạo thành phạm vi nghiệp vụ cốt lõi của hệ thống. Những cải thiện như tệp đính kèm cho đơn nghỉ trong môi trường demo local, báo cáo nâng cao, lịch theo phòng ban hoặc tinh chỉnh giao diện theo phản hồi được thực hiện dựa trên cùng mô hình actor, dữ liệu và luồng xử lý.
...
Một số cải thiện phục vụ demo local, chẳng hạn tệp đính kèm cho đơn nghỉ hoặc tinh chỉnh giao diện theo phản hồi sử dụng, được mô tả như phần mở rộng tự nhiên của các nhóm chức năng đã nêu: quản lý đơn nghỉ, quản lý dữ liệu liên quan, phân quyền truy cập, tổng hợp dữ liệu và lưu vết thao tác.
...
\subsection{Ma trận quyền theo vai trò}
Ma trận dưới đây tóm tắt phạm vi quyền của các nhóm người dùng chính. Bảng chỉ trình bày ở mức nghiệp vụ để làm rõ ranh giới trách nhiệm; kiểm tra quyền chi tiết vẫn được thực hiện ở backend theo vai trò và phạm vi dữ liệu.

\begin{longtable}{p{0.28\textwidth}p{0.12\textwidth}p{0.12\textwidth}p{0.12\textwidth}p{0.12\textwidth}}
\toprule
\textbf{Nghiệp vụ} & \textbf{Employee} & \textbf{Manager} & \textbf{HR} & \textbf{Admin} \\
\midrule
\endfirsthead
\toprule
\textbf{Nghiệp vụ} & \textbf{Employee} & \textbf{Manager} & \textbf{HR} & \textbf{Admin} \\
\midrule
\endhead
Nộp/sửa/hủy đơn của mình & Có & Có & Có & Có \\
Duyệt/từ chối đơn & Không & Cấp dưới & Toàn tổ chức & Toàn tổ chức \\
Xem lịch nghỉ & Phòng ban & Nhóm & Toàn tổ chức & Toàn tổ chức \\
Quản lý người dùng/phòng ban & Không & Không & Hạn chế & Có \\
Quản lý loại nghỉ/ngày lễ/quỹ phép & Không & Không & Có & Có \\
Xem dashboard/báo cáo tổ chức & Không & Nhóm & Có & Có \\
Xem/tải tệp đính kèm & Theo đơn được phép xem & Theo đơn được phép xem & Theo đơn được phép xem & Theo đơn được phép xem \\
\bottomrule
\end{longtable}
...
\begin{tikzpicture}[
  ...
  \node[entity] (attachments) at (0,-4.05) {
    \centering\textbf{\texttt{attachments}}\par
    \raggedright
    \texttt{leave\_request\_id -> leave\_requests}\\
    \texttt{uploaded\_by -> users}
  };
  ...
  \draw[rel] (attachments.north west) to[out=140,in=-95] (requests.south west);
  \draw[softrel] (attachments.north east) to[out=65,in=-95] (users.south east);
...
\texttt{attachments} & Lưu metadata của tệp đính kèm cho đơn nghỉ trong môi trường demo local; file vật lý được lưu ngoài database trong Docker named volume. \\
...
Trong hệ thống, migration nền tạo các bảng cốt lõi như phòng ban, người dùng, loại nghỉ, quỹ phép, ngày lễ, đơn nghỉ, lịch sử phê duyệt, audit log và refresh token. Các migration sau bổ sung dữ liệu tham chiếu hoặc mở rộng schema cho những năng lực như thông báo, chuyển tiếp quỹ phép và metadata tệp đính kèm local-only. Khi cần thay đổi cấu trúc dữ liệu, hệ thống tạo migration mới thay vì sửa migration đã áp dụng để tránh lỗi checksum và bảo đảm tính lặp lại của quá trình khởi tạo database.
...
Bảng sau tóm tắt các quy tắc nghiệp vụ quan trọng được backend kiểm soát. Việc đặt các quy tắc này ở tầng service giúp hệ thống giữ tính đúng đắn ngay cả khi request không đi qua giao diện frontend.

\begin{longtable}{p{0.25\textwidth}p{0.61\textwidth}}
\toprule
\textbf{Nhóm quy tắc} & \textbf{Mô tả} \\
\midrule
\endfirsthead
\toprule
\textbf{Nhóm quy tắc} & \textbf{Mô tả} \\
\midrule
\endhead
Thời gian nghỉ & Ngày bắt đầu không được nằm trong quá khứ, ngày kết thúc không được trước ngày bắt đầu, và lựa chọn nửa ngày phải tạo ra số ngày nghỉ hợp lệ lớn hơn 0. \\
Tính ngày nghỉ & Hệ thống loại trừ cuối tuần và ngày lễ khi tính số ngày nghỉ; trường hợp nửa ngày được quy đổi theo quy tắc thống nhất ở backend. \\
Chồng lấn đơn & Một người dùng không được có các đơn nghỉ hợp lệ chồng lấn trong cùng khoảng thời gian, nhằm tránh trùng dữ liệu lịch và quỹ phép. \\
Quỹ phép & Với loại nghỉ yêu cầu quỹ phép, hệ thống kiểm tra số ngày còn lại khi tạo/cập nhật và kiểm tra lại khi duyệt để tránh sai lệch do dữ liệu thay đổi. \\
Chuyển trạng thái & Chỉ đơn \texttt{PENDING} mới được duyệt hoặc từ chối. Đơn đã \texttt{REJECTED} hoặc \texttt{CANCELLED} không được đưa lại về \texttt{PENDING}. \\
Hủy đơn và hoàn quỹ & Đơn có thể bị hủy theo điều kiện nghiệp vụ. Nếu đơn đã trừ quỹ phép, hệ thống hoàn lại phần quỹ tương ứng khi hủy hợp lệ. \\
Tệp đính kèm & Tệp đính kèm chỉ phục vụ demo/test local; người tạo đơn chỉ upload/xóa khi đơn còn \texttt{PENDING}; người có quyền xem đơn mới được xem hoặc tải file. \\
\bottomrule
\end{longtable}
...
Backend sử dụng Java 21 và Spring Boot 3.3 \cite{spring-boot}. Spring Security đảm nhiệm xác thực và phân quyền \cite{spring-security}, Spring Data JPA hỗ trợ truy cập dữ liệu, Flyway quản lý migration cơ sở dữ liệu \cite{flyway} và PostgreSQL là hệ quản trị cơ sở dữ liệu quan hệ chính \cite{postgresql}. Các thành phần nghiệp vụ được tổ chức theo feature để giữ mã nguồn gần với domain của hệ thống.

Frontend sử dụng React 18 \cite{react}, TypeScript, Vite \cite{vite}, Tailwind CSS \cite{tailwind} và các component theo hướng tái sử dụng. TanStack Query được dùng cho server state \cite{tanstack-query}, React Hook Form \cite{react-hook-form} và Zod \cite{zod} được dùng cho form validation. Các request từ frontend đi qua lớp API client thay vì gọi trực tiếp trong component, giúp giao diện dễ kiểm thử và dễ thay đổi khi backend API điều chỉnh.

Môi trường phát triển và chạy thử được container hóa bằng Docker Compose \cite{docker}. Cách này giúp backend, frontend và database chạy trong cùng một cấu hình có thể lặp lại. CI được dùng để kiểm tra các bước chất lượng như build, test và lint trước khi hợp nhất thay đổi.
